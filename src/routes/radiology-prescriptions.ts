...
router.get('/appointment/:appointmentId', async (req, res) => {
  try {
    ...
    const patientResult = await pool.query(patientQuery, [appointmentId]);
    ...
    const servicesResult = await pool.query(servicesQuery, [appointmentId]);
...
router.patch('/:prescriptionId/test-conducted', async (req, res) => {
  try {
    ...
    const result = await pool.query(updateQuery, [test_conducted, prescriptionId]);
    ...
    const prescriptionResult = await pool.query(getQuery, [prescriptionId]);
...
router.patch('/:prescriptionId/status', async (req, res) => {
  try {
    ...
    const result = await pool.query(updateQuery, [status, prescriptionId]);
...
router.patch('/:prescriptionId/payment-status', async (req, res) => {
  try {
    ...
    const result = await pool.query(updateQuery, [payment_status, prescriptionId]);
