...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [name, price || null]);
...
router.put('/:serviceId', async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [name, price || null, serviceId]);
...
router.delete('/:serviceId', async (req, res) => {
  try {
    ...
    const templateCheckResult = await pool.query(templateCheckQuery, [serviceId]);
...
router.post('/:serviceId/templates', async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [serviceId, template_name, template_structure]);
    res.status(201).json(result.rows[0]);
  } catch (error) {
...
router.put('/:serviceId/templates/:templateId', async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [template_name, template_structure, templateId, serviceId]);
...
router.delete('/:serviceId/templates/:templateId', async (req, res) => {
  try {
    ...
    const result = await pool.query(deleteQuery, [templateId, serviceId]);
