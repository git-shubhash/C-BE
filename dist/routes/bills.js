...
router.post('/razorpay/verify', async (req, res) => {
    ...
    if (generated_signature === razorpay_signature) {
        ...
        try {
            ...
            await connection_1.default.query(`UPDATE bills SET payment_status = 'paid', transaction_id = $1 WHERE appointment_id = $2`, [razorpay_payment_id, appointment_id]);
            res.json({ valid: true });
        }
        catch (err) {
...
router.post('/sms/send', async (req, res) => {
    try {
      ...
      ORDER BY b.created_at DESC
    `;
        const billResult = await connection_1.default.query(billQuery, [appointment_id]);
    ...
    `;
        ...
        try {
            const existingResult = await cloudinary_1.v2.api.resource(existingPublicId, {
                resource_type: 'raw'
            });
            ...
            try {
                ...
                const existingTextResult = await cloudinary_1.v2.api.resource(textPublicId, {
                    resource_type: 'raw'
                });
...
Thank you for choosing CURA Pharmacy!
          `;
                    try {
                        const textPublicId = `${existingPublicId}_text`;
                        const textResult = await cloudinary_1.v2.uploader.upload(`data:text/plain;base64,${Buffer.from(billText).toString('base64')}`, {
                            resource_type: 'raw',
                            format: 'txt',
