...
router.get('/service/:patientServiceId', async (req, res) => {
    ...
    `;
        const serviceResult = await connection_1.default.query(serviceQuery, [patientServiceId]);
    ...
    `;
        const allTestsResult = await connection_1.default.query(allTestsQuery, [serviceTypeId]);
    ...
    `;
        const existingTestsResult = await connection_1.default.query(existingTestsQuery, [patientServiceId]);
    ...
    `;
        const result = await connection_1.default.query(finalQuery, [patientServiceId]);
        res.json(result.rows);
    }
    catch (error) {
...
router.get('/:patientTestId/results', async (req, res) => {
    ...
    `;
        const result = await connection_1.default.query(query, [patientTestId]);
        res.json(result.rows);
    }
    catch (error) {
...
router.post('/:patientTestId/results', async (req, res) => {
    try {
      ...
      RETURNING *
    `;
        const result = await connection_1.default.query(query, [patientTestId, result_value]);
...
router.patch('/results/:resultId', async (req, res) => {
    try {
      ...
      RETURNING *
    `;
        const result = await connection_1.default.query(query, [result_value, resultId]);
...
router.patch('/service/:patientServiceId/complete', async (req, res) => {
    try {
      ...
      RETURNING *
    `;
        const result = await connection_1.default.query(query, [patientServiceId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Patient service not found' });
        }
        res.json(result.rows[0]);
