...
router.get('/:appointment_id', async (req, res) => {
    ...
    `;
        const result = await connection_1.default.query(query, [appointment_id]);
...
router.delete('/:appointment_id', async (req, res) => {
    try {
      ...
      WHERE appointment_id = $1
    `;
        const checkResult = await connection_1.default.query(checkQuery, [appointment_id]);
      ...
      RETURNING *
    `;
        const result = await connection_1.default.query(deleteQuery, [appointment_id]);
...
router.post('/sms/send', async (req, res) => {
    try {
      ...
      ORDER BY p.created_at DESC
    `;
        const prescriptionResult = await connection_1.default.query(prescriptionQuery, [appointment_id]);
    ...
    `;
        ...
        try {
            const existingResult = await cloudinary_1.v2.api.resource(existingPublicId, { resource_type: 'raw' });
            ...
            try {
                ...
                const existingTextResult = await cloudinary_1.v2.api.resource(textPublicId, { resource_type: 'raw' });
                ...
                try {
                    ...
                    const result = await cloudinary_1.v2.uploader.upload(`data:application/pdf;base64,${base64PDF}`, {
                        resource_type: 'raw',
                        format: 'pdf',
                    ...
                    try {
                        const textPublicId = `${existingPublicId}_text`;
                        const textResult = await cloudinary_1.v2.uploader.upload(`data:text/plain;base64,${Buffer.from(prescriptionText).toString('base64')}`, {
                            resource_type: 'raw',
                            format: 'txt',
